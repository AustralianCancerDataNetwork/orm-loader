...
class _FakeBackend:
    """Captures every call instead of touching a real connection/database,
    so these tests prove forwarding/ordering, not DDL execution (already
    covered by the postgres backend's own tests)."""
...
def test_resolve_mv_refresh_order_linear_dependency():
    ordered = resolve_mv_refresh_order([_MvB, _MvA])

    assert ordered == [_MvA, _MvB]

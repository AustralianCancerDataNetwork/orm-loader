...
def test_fake_backend_inherits_default_materialized_view_drop_and_index_behavior():
    """FakeBackend adds zero code for drop_materialized_view/
    create_materialized_view_index: these are non-abstract on DatabaseBackend
    precisely so an unsupporting (or third-party, pre-existing) backend
    subclass gets the same clean NotImplementedError as the two abstract
    methods without needing an override at all."""
    backend = FakeBackend()
